...
def apply_photometric_augmentation(image, args):
    if not args.augment_photometric or random.random() > args.aug_prob:
        return image
    ...
    if args.brightness > 0:
        factor = random.uniform(1.0 - args.brightness, 1.0 + args.brightness)
        image = image * factor

    if args.contrast > 0:
        factor = random.uniform(1.0 - args.contrast, 1.0 + args.contrast)
        mean = image.mean(dim=(1, 2), keepdim=True)
        image = (image - mean) * factor + mean

    if args.gamma > 0:
        gamma_value = random.uniform(1.0 - args.gamma, 1.0 + args.gamma)
        image = image.clamp(0, 1).pow(gamma_value)

    if args.noise_std > 0:
        std = random.uniform(0.0, args.noise_std)
        image = image + torch.randn_like(image) * std

    if args.blur_prob > 0 and random.random() < args.blur_prob:
        image = TF.gaussian_blur(image, kernel_size=[3, 3], sigma=[0.1, args.blur_sigma])
...
def main():
    ...
    parser.add_argument("--augment-photometric", action="store_true", help="Apply brightness, contrast, gamma, noise, and blur augmentation during training.")
    parser.add_argument("--aug-prob", type=float, default=0.8, help="Probability of applying photometric augmentation per image.")
    parser.add_argument("--brightness", type=float, default=0.15, help="Brightness jitter range.")
    parser.add_argument("--contrast", type=float, default=0.15, help="Contrast jitter range.")
    parser.add_argument("--gamma", type=float, default=0.15, help="Gamma jitter range.")
    parser.add_argument("--noise-std", type=float, default=0.01, help="Maximum Gaussian noise standard deviation.")
    parser.add_argument("--blur-prob", type=float, default=0.15, help="Probability of Gaussian blur.")
    parser.add_argument("--blur-sigma", type=float, default=1.0, help="Maximum Gaussian blur sigma.")
    ...
    with log_path.open("a", newline="") as f:
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            for step, (images, targets) in enumerate(progress, start=1):
                images = [apply_photometric_augmentation(img, args).to(device) for img in images]
            ...
            if args.val_every > 0 and epoch % args.val_every == 0:
                ...
                if improved:
                    ...
                    best_info = {
                        ...
                        "batch_size": args.batch_size,
                        "lr": args.lr,
                        "augment_photometric": args.augment_photometric,
